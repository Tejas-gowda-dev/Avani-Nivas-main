        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          {PARTNERS.map((partner, idx) => (
            <ScrollReveal
              key={partner.id}
              delay={idx * 0.05}
              className="bg-white p-7 rounded-2xl border border-[#E0D8CC] shadow-xs flex flex-col justify-between hover:shadow-xl hover:border-[#0B2545]/60 transition-all duration-300"
            >
              <div>
                <span className="inline-block px-2.5 py-0.5 rounded text-[11px] font-bold uppercase tracking-wider bg-[#0B2545]/10 text-[#0B2545] mb-3">
                  {partner.category}
                </span>
                <h2 className="text-lg font-bold text-[#252525] font-serif">
                  {partner.name}
                </h2>
                <p className="text-xs font-semibold text-[#0B2545] mt-1 mb-3">
                  {partner.role}
                </p>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {COLLABORATION_TYPES.map((type, idx) => (
              ...
              >
                <div className="w-10 h-10 rounded-xl bg-[#0B2545]/10 text-[#0B2545] flex items-center justify-center font-bold mb-4">
                  0{idx + 1}
                </div>
                <h3 className="text-base font-bold text-[#252525] font-serif mb-2">{type.title}</h3>
                ...
                <ul className="space-y-2 pt-4 border-t border-[#F2ECE4]">
                  {type.benefits.map((b, i) => (
                    <li key={i} className="flex items-center gap-2 text-xs text-[#252525]">
